...
%we're interested in
[Position,Uxreal,Uximag,Uzreal,Uzimag,Uyreal,Uyimag] = importfile4('D:\OneDrive - Imperial College London\brasssteelbrass.txt',2, 52)
...
brassEls = find(ey <bottom | ey>top);

m.elTypes = cell(2,1);
m.matTypeRefs(brassEls) = 2 
m.elTypeRefs(brassEls) = 2
m.matTypes = cell(2,1);
...
m.matTypes{2}.paramsType = 0;
m.matTypes{2}.paramValues = [Eb,nub,rhob];
%% 
...
% %% Saving output
savePogoInp('brasssteelbrass1mm.pogo-inp',m);
